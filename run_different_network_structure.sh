...
methods=("AV_temperature" "active_query" "Core_set" 'certainty' 'OpenMax' 'BGADL' 'random' "BADGE_sampling")
...
do
    ...
    do
        ...
        do
            for i in 10
            do
                CUDA_VISIBLE_DEVICES=3 python NEAT_main.py --gpu 3 --k $i --save-dir log_AL/ --weight-cent 0 --query-strategy $method --init-percent 8 --known-class 20 --query-batch $j --seed 1 --model $structure --known-T 0.5 --unknown-T 0.5 --modelB-T 1 --dataset cifar100 &
                CUDA_VISIBLE_DEVICES=2 python NEAT_main.py --gpu 2 --k $i --save-dir log_AL/ --weight-cent 0 --query-strategy $method --init-percent 8 --known-class 20 --query-batch $j --seed 2 --model $structure --known-T 0.5 --unknown-T 0.5 --modelB-T 1 --dataset cifar100 &
